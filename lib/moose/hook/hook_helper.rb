module Moose
  module Hook
    module HookHelper
      ...
      def add_before_teardown_hook(&block)
        create_before_teardown_hook_from(block: block)
      end

      def add_after_teardown_hook(&block)
        create_after_teardown_hook_from(block: block)
      end
      ...
      def create_after_teardown_hook_from(collection: hook_collection, block:)
        collection.add_after_teardown_hook(
          After.new(block: block)
        )
      end

      def create_before_teardown_hook_from(collection: hook_collection, block:)
        collection.add_before_teardown_hook(
          Before.new(block: block)
        )
      end
